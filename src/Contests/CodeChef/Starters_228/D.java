...
public class D {
    ...
    static void solve() throws IOException {
        ...
        long mPowN = modPow(m, n, MOD);

        long first = computeFirstTerm(n, m, mPowN);
        long sumPow = computeSumOfPowers(n, m);
        long ans = computeAnswer(first, sumPow);

        out.println(ans);
    }

    // N * M^N * (M + 1) / 2
    static long computeFirstTerm(int n, int m, long mPowN) {
        long result = (n % (long) MOD) * mPowN % MOD;
        result = result * ((m + 1L) % MOD) % MOD;
        result = result * INV2 % MOD;
        return result;
    }

    // sum_{x=1..M} x^N
    static long computeSumOfPowers(int n, int m) {
        long sumPow = 0;
        for (int x = 1; x <= m; x++) {
            sumPow += modPow(x, n, MOD);
            if (sumPow >= MOD)
                sumPow -= MOD;
        }
        return sumPow;
    ...
    // first - sumPow (mod MOD)
    static long computeAnswer(long first, long sumPow) {
        long ans = first - sumPow;
        if (ans < 0)
            ans += MOD;
        return ans;
    }
